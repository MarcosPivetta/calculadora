...
export class CalculadoraComponent implements OnInit {
  ...
  concatenarNumero(numAtual: string, numConcat: string): string {
    ...
    // primeiro dífito é '.', concatena '0' antes do ponto
    if (numConcat === '.' && numAtual === '') {
      return '0.';
    }

    // caso '.' difitado e já contanha um '.', apenas retorna
   ...
   * operacação anterior, e define a nova operação.
   *
   * @param string operacao
   * @return void
   */
  definirOperacao(operacao: string): void {
    // apenas define a opercaão caso não exista uma
    if (this.operacao === null) {
      this.operacao = operacao;
      return;
    }

    /** caso opercação definida e número 2 selecionado,
